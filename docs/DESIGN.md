...
2. **Per-host shim (progressive enhancement)** — instruction-file injection (CLAUDE.md / AGENTS.md / GEMINI.md, `--append-system-prompt`) everywhere; **native lifecycle + pre-tool hooks on all four hosts** (Claude `SessionStart/Stop/SessionEnd/PreToolUse`; Gemini `SessionStart/AfterAgent/SessionEnd/BeforeTool`; Codex `SessionStart/Stop/PreToolUse`, interactive-only; OpenCode bus plugin) → live capture + tool-gating, each wired from real captured payloads (exp-11). Capture *depth* still varies (Gemini's log is prompt-only; Claude/Codex carry tool spans) — that's a per-host transcript-richness gap, not a hook-surface gap.
...
1. **Stage 1 — host-agnostic wrapper (building now):** wrap the hosts users already run — Claude Code, Gemini CLI, Codex (+ OpenCode as a peer). Faculties served via instruction-file injection + filesystem; live capture and the guardrails gate wherever the host's hook surface allows. The pitch here: *your existing subscription goes further* (§7 pricing edge preserved). Current state (exp-11): post-hoc capture = all 4 hosts; **live capture = all 4 hosts**; **gate = Claude Code, Gemini CLI, Codex** (Codex interactive-only — `codex exec` fires no hooks) — wired from real captured hook payloads and dogfooded end-to-end. OpenCode's `tool.execute.before` gate is the one remaining rung.

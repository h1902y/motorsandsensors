...
2. **Stage 2 — double down on OpenCode (stage-1 host parity reached; launcher shipped 2026-06-11):** **`mns code` is built** — one command ensures the faculty home, detects + installs OpenCode on demand (zero-dep: a runtime peer, never an npm dependency), wires the mns plugin (capture + gate + grounding), and launches the real `opencode` interactively (configure + launch, never fork or drive). The one-step door for users who run no host. *Still to come in stage 2:* the **efficiency benchmark** (§2) at scale — token-per-task with/without faculties on cheap models — and optional MNS provider/credits onboarding. Not a fork — a distribution.

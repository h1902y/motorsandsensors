...
1. **Stage 1 — host-agnostic wrapper (building now):** wrap the hosts users already run — Claude Code, Gemini CLI, Codex (+ OpenCode as a peer). Faculties served via instruction-file injection + filesystem; live capture and the guardrails gate wherever the host's hook surface allows. The pitch here: *your existing subscription goes further* (§7 pricing edge preserved). Current state: post-hoc capture = all 4 hosts; live + gate = Claude Code & OpenCode only; Gemini/Codex live signals are the open build item (real-wire-data rule applies — observe their hook/notify surfaces before wiring).

...
- **Workbench packaging is default-full, ADK-style (decided 2026-06-12):** `@zuzuucodes/web` ships as an **`optionalDependencies`** entry of `@zuzuucodes/cli` — `npm i -g @zuzuucodes/cli` bundles the workbench (one install, web included, the Google-ADK product statement); `--omit=optional` is the **light** install (CLI only; `zuzuu web` installs on demand). The CLI's `dependencies` stays empty and its core never imports the web package, so the zero-dep resilience guarantee holds: a failed native build (node-pty) degrades the workbench, never the CLI. `zuzuu web` resolves bundled copy → PATH binary → install-on-demand. Separate repos/pipelines remain (the bundle is a manifest-level union, not a code merge).

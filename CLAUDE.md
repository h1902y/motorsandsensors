...
- Product experience (decided 2026-06-12, DESIGN §13): the home is **hidden `.zuzuu/`** (reversed the visible-`agent/` decision with cause — clash risk + brownfield clutter; transparency = porcelain: `zz status`/`explain`/`digest` + plain-text files + the human gate, the `.git` model) · six workstreams in dependency order (home/onboarding → workbench → faculty-health UX → marketplace templates → tasks faculty → benchmark) · the workbench middle pane = **embedded terminal** (xterm.js running the real host CLI), never a custom chat loop · **zuzuu-web evolves into the workbench** (one web surface) · workbench packaging = **default-full via `optionalDependencies`** (`@zuzuucodes/web` bundles with the CLI; `--omit=optional` = light; `dependencies` stays empty — the zero-dep guarantee is about the CLI core, not the install footprint) · positioning = AI-first directory for *any* folder-based work, coding is the first vertical · philosophy = **enhance, never reinvent**.

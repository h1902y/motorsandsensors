...
function overview() {
  return [
    ...
    LOOP_DIAGRAM,
    '',
    '  Nothing graduates without you. Each approval mints a per-module',
    '  generation — an immutable lineage you can roll back to; a checkpoint',
    '  composes them for whole-brain coherence.',
    '',
    'Get in the loop:',
    '  zuzuu inbox                       what is pending your approval',
    '  zuzuu review                      walk each proposal: approve / reject / edit',
    '  zuzuu module <m> generations      a module\'s generation lineage (rollback anytime)',
    '  zuzuu checkpoint list             whole-brain checkpoints (compose all modules)',
    '',
...
function graduation() {
  return [
    ...
    '4. Each review mints a GENERATION PER AFFECTED MODULE — an immutable,',
    '   content-addressed snapshot of that module\'s items (its own lineage).',
    '5. Rollback is flipping a pointer: `zuzuu module <m> generation rollback <id>`',
    '   restores ONE module by content; `zuzuu checkpoint rollback <id>` restores',
    '   the whole brain to a composed checkpoint. Your approvals are never lost.',
    '',
    'Inspect: `zuzuu module <m> generations` · `zuzuu checkpoint list`.',
  ].join('\n');
}
